...
const findById = async (id) => {
    const record = await prisma_1.default.employee.findUnique({ where: { id } });
    if (!record)
...
const create = async (data) => {
    return await prisma_1.default.employee.create({ data });
};
...
const remove = async (id) => {
    return await prisma_1.default.employee.delete({
        where: { id }
    });
};
